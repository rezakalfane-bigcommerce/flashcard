...
import { createExpressionAction, saveExpressionAction, translateExpressionAction } from "./actions";
      ...
      <div className="mx-auto grid max-w-6xl gap-6 py-7 lg:grid-cols-[1fr_310px]">
        <section className="rounded-3xl border border-[#1d4d58]/15 bg-white p-6 shadow-sm sm:p-8">
          ...
          <form action={isNew ? createExpressionAction : saveExpressionAction} className="space-y-5">
            ...
            <div className="grid gap-5 sm:grid-cols-2"><SelectField label="Translation status" name="translationStatus" defaultValue={phrase?.translationStatus ?? "missing"} options={["missing", "draft", "translated", "reviewed"]} /><SelectField label="Editorial review" name="reviewStatus" defaultValue={phrase?.reviewStatus ?? "unreviewed"} options={["unreviewed", "needs_review", "approved", "rejected"]} /></div>
        ...
        <aside className="space-y-5">
          {!isNew && <section className="rounded-3xl bg-[#15292d] p-6 text-white"><p className="mono text-[10px] uppercase tracking-[.18em] text-white/50">AI translation</p><h2 className="display mt-2 text-3xl">Create a draft</h2><p className="mt-3 text-sm leading-6 text-white/60">Generates meaning, literal translation, and context. Existing text is replaced and marked for review.</p><div className="mt-6 grid gap-3"><AiButton id={phrase.id} provider="openai" label="Translate with OpenAI" /><AiButton id={phrase.id} provider="gemini" label="Translate with Gemini" /></div><p className="mono mt-4 text-[9px] leading-4 text-white/35">Via Vercel AI Gateway. Human approval remains required.</p></section>}
...
function AiButton({ id, provider, label }: { id: number; provider: "openai" | "gemini"; label: string }) { return <form action={translateExpressionAction}><input type="hidden" name="id" value={id} /><input type="hidden" name="provider" value={provider} /><button className="w-full rounded-xl border border-white/15 bg-white/10 px-4 py-3 text-sm font-semibold hover:bg-white/15">{label}</button></form>; }
