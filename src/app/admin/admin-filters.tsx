...
export function AdminFilters({ initial, sources, totalLevels, basePath = "/admin" }: Props & { basePath?: string }) {
  ...
  return (
    <form key={[initial.query, initial.source, initial.translationStatus, initial.reviewStatus, initial.level, initial.sort, initial.direction].join("|")} onSubmit={remember} className="grid gap-3 rounded-3xl border border-[#1d4d58]/15 bg-white/70 p-4 shadow-sm md:grid-cols-[minmax(240px,2fr)_repeat(6,minmax(120px,1fr))_auto_auto]">
      ...
      <Select name="translationStatus" defaultValue={initial.translationStatus}><option value="">Any translation</option><option value="missing">Missing</option><option value="partly_missing">Partly missing</option><option value="draft">Draft</option><option value="translated">Translated</option><option value="reviewed">Reviewed</option><option value="without_audio">Without audio</option></Select>
