...
export function FlashcardApp({ initialData, canChangeLevel = false }: { initialData: DashboardData; canChangeLevel?: boolean }) {
  ...
  const [locale, setLocale] = useState<PlayerLocale>("fr");
  ...
  useEffect(() => {
    const saved = window.localStorage.getItem("player-locale");
    if (saved === "en" || saved === "fr") queueMicrotask(() => setLocale(saved));
  }, []);

  useEffect(() => {
    window.localStorage.setItem("player-locale", locale);
  }, [locale]);
    ...
    <main className="min-h-screen px-5 py-6 md:px-10 md:py-8">
      <header className="mx-auto flex max-w-6xl items-center justify-between border-b border-[#1d4d58]/20 pb-5">
        <div className="flex items-baseline gap-3">
          ...
          <span className="mono hidden text-[10px] uppercase tracking-[.22em] text-[#78979c] sm:inline">{m.header.brandTagline}</span>
        </div>
        <div className="flex items-center gap-2">{canChangeLevel && <Link href="/admin" className="rounded-full px-4 py-2.5 text-sm font-semibold text-[#1d4d58] hover:bg-[#d9eeec]">{m.header.admin}</Link>}<label className="sr-only" htmlFor="player-language">{m.header.language}</label><span className="relative inline-flex"><select id="player-language" value={locale} onChange={(event) => setLocale(event.target.value as PlayerLocale)} className="appearance-none rounded-full border border-[#1d4d58]/15 bg-white/60 py-2 pl-3 pr-11 text-xs font-semibold text-[#1d4d58] outline-none focus:ring-2 focus:ring-[#b7d86a]"><option value="fr">🇫🇷 Français</option><option value="en">🇬🇧 English</option></select><span aria-hidden="true" className="pointer-events-none absolute right-3 top-1/2 h-2 w-2 -translate-y-1/2 rotate-45 border-b-2 border-r-2 border-[#1d4d58]" /></span><UserButton /></div>
      </header>

      <section className={`mx-auto grid max-w-6xl gap-10 py-10 ${quizMode ? "grid-cols-1 lg:py-16" : "lg:grid-cols-[1fr_300px] lg:py-16"}`}>
        {quizMode ? <QuizMode phrases={initialData.phrases} level={initialData.study.currentLevel} totalLevels={initialData.study.totalLevels} locale={locale} isChangingLevel={isPending} onExit={() => setQuizMode(false)} onPass={() => goToLevel(initialData.study.currentLevel + 1)} /> : <div>
          <div className="mb-6 flex items-end justify-between">
            <div>
            <p className="mono mb-2 text-[10px] uppercase tracking-[.24em] text-[#78979c]">{m.level.todayPractice}</p>
            </div>
            <div className="text-right">
              {canChangeLevel && <div className="flex items-center justify-end gap-2">
                <button type="button" aria-label={m.header.previousLevel} title={`${m.header.previousLevel} (${m.header.adminTesting})`} onClick={() => goToLevel(initialData.study.currentLevel - 1)} disabled={isPending || initialData.study.currentLevel === 1} className="grid h-8 w-8 place-items-center rounded-full border border-[#1d4d58]/20 text-[#1d4d58] transition hover:bg-[#d9eeec] disabled:cursor-not-allowed disabled:opacity-30">←</button>
                <span className="mono text-xs font-semibold text-[#1d4d58]">{translate(m.level.counter, { current: String(initialData.study.currentLevel).padStart(2, "0"), total: String(initialData.study.totalLevels).padStart(2, "0") })}</span>
                <button type="button" aria-label={m.header.nextLevel} title={`${m.header.nextLevel} (${m.header.adminTesting})`} onClick={() => goToLevel(initialData.study.currentLevel + 1)} disabled={isPending || initialData.study.currentLevel === initialData.study.totalLevels} className="grid h-8 w-8 place-items-center rounded-full border border-[#1d4d58]/20 text-[#1d4d58] transition hover:bg-[#d9eeec] disabled:cursor-not-allowed disabled:opacity-30">→</button>
              </div>}
              <span className="mt-1 block text-xs text-[#78979c]">{translate(m.level.progressing, { mastered: initialData.study.levelMastered, total: initialData.study.levelTotal })}{canChangeLevel ? ` · ${m.level.adminControls}` : ""}</span>
              <button type="button" onClick={() => setQuizMode(true)} className="mt-3 rounded-full border border-[#1d4d58]/20 bg-white/60 px-4 py-2 text-xs font-semibold text-[#1d4d58] transition hover:bg-[#d9eeec]">{m.level.quizThisLevel}</button>
            </div>
          </div>
            ...
            <div className="card-scene relative h-[400px] w-full sm:h-[430px] lg:h-[580px]">
              <button onClick={() => setFlipped(!flipped)} aria-label={flipped ? "Show Icelandic phrase" : "Reveal translation"} className={`card-inner relative h-full w-full text-left focus:outline-none focus-visible:ring-4 focus-visible:ring-[#b7d86a] ${flipped ? "flipped" : ""}`}>
                <article className="card-face absolute inset-0 flex flex-col justify-between overflow-hidden rounded-[2rem] bg-[#1d4d58] p-8 text-white shadow-[0_25px_70px_-28px_rgba(21,41,45,.7)] sm:p-12">
                  ...
                  <div className="flex items-center justify-between gap-3">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <span className="mono rounded-full bg-white/10 px-3 py-1 text-[10px] uppercase tracking-[.14em] text-[#d9eeec]">{m.card.complexity} · {phrase.complexity}</span>
                    </div>
                    <span className="mono text-[10px] tracking-widest text-white/50">{m.card.direction}</span>
                  </div>
                  ...
                  <div className="flex items-end justify-between border-t border-white/15 pt-5">
                    <p className="mono text-xs text-[#d9eeec]">{m.card.expressionLabel}</p>
                    <p className="text-xs text-white/55">{m.card.tapToTurn}</p>
                  </div>
                </article>
                <article className="card-face card-back absolute inset-0 flex flex-col justify-between overflow-hidden rounded-[2rem] border border-[#1d4d58]/15 bg-[#d9eeec] p-8 shadow-[0_25px_70px_-28px_rgba(21,41,45,.4)] sm:p-12">
                  <div className="pr-64 sm:pr-72"><span className="mono block whitespace-nowrap text-[10px] uppercase tracking-[.2em] text-[#1d4d58]">{m.card.englishNotes}</span><div className="mt-2 flex flex-nowrap gap-2 overflow-hidden"><span className="mono shrink-0 rounded-full border border-[#1d4d58]/15 px-2.5 py-1 text-[9px] uppercase tracking-[.14em] text-[#78979c]">{m.card.complexity} · {phrase.complexity}</span><span className="mono shrink-0 rounded-full border border-[#1d4d58]/15 px-2.5 py-1 text-[9px] uppercase tracking-[.14em] text-[#78979c]">{m.card.source} · {phrase.source}</span></div></div>
                  <div className="min-h-0 flex-1 overflow-y-auto pb-3 pr-2 pt-7">
                    <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_minmax(320px,1.25fr)]">
                      <div className="space-y-5">
                        <div><p className="mono text-[9px] uppercase tracking-[.18em] text-[#78979c]">{m.card.icelandic}</p><p lang="is" className="display mt-2 text-xl font-medium leading-tight text-[#1d4d58] sm:text-2xl">{phrase.icelandic}</p></div>
                        <div><p className="mono text-[9px] uppercase tracking-[.18em] text-[#78979c]">{m.card.meaning}</p><p className="display mt-1 text-3xl leading-tight text-[#15292d] sm:text-4xl">{phrase.meaning || m.card.translationMissing}</p></div>
                        <div><p className="mono text-[9px] uppercase tracking-[.18em] text-[#78979c]">{m.card.literal}</p><p className="mt-1 text-sm font-semibold text-[#1d4d58]">{phrase.literal || m.card.notAvailable}</p></div>
                      </div>
                      <div className="border-t border-[#1d4d58]/15 pt-5 lg:border-l lg:border-t-0 lg:pl-7 lg:pt-0"><p className="mono text-[9px] uppercase tracking-[.18em] text-[#78979c]">{m.card.why}</p><p className="mt-2 text-sm leading-6 text-[#1d4d58]/75">{phrase.why || m.card.notAvailable}</p></div>
                    </div>
                  </div>
                  <p className="mt-5 shrink-0 text-xs text-[#1d4d58]/60">{m.card.tapToSeeAgain}</p>
                </article>
              </button>
              {phrase.audioUrl && <AudioPlayButton audioUrl={phrase.audioUrl} resetKey={phrase.id} messages={m} />}
            </div>
          )}

          <div className={`mt-6 grid grid-cols-2 gap-3 transition ${flipped ? "opacity-100" : "pointer-events-none opacity-35"}`}>
            <button onClick={() => rate(false)} disabled={savingChoice !== null} aria-busy={savingChoice === false} className="inline-flex items-center justify-center gap-2 rounded-2xl border border-[#1d4d58]/20 bg-white/70 py-4 font-semibold text-[#1d4d58] hover:bg-white disabled:cursor-wait disabled:opacity-70">{savingChoice === false && <Spinner />} {savingChoice === false ? m.review.saving : m.review.stillLearning}</button>
            <button onClick={() => rate(true)} disabled={savingChoice !== null} aria-busy={savingChoice === true} className="inline-flex items-center justify-center gap-2 rounded-2xl bg-[#b7d86a] py-4 font-semibold text-[#15292d] hover:bg-[#c5e479] disabled:cursor-wait disabled:opacity-70">{savingChoice === true && <Spinner />} {savingChoice === true ? m.review.saving : m.review.remembered}</button>
          </div>
        </div>}
        ...
        {!quizMode && <aside className="flex flex-col gap-5 lg:pt-20">
          <div className="rounded-3xl border border-[#1d4d58]/15 bg-white/55 p-6 backdrop-blur">
            <p className="mono text-[10px] uppercase tracking-[.2em] text-[#78979c]">{m.notebook.title}</p>
            <div className="mt-6 grid grid-cols-3 gap-3 lg:grid-cols-1">
              <Stat value={initialData.stats.seen} label={m.notebook.phrasesSeen} />
              <Stat value={initialData.stats.mastered} label={m.notebook.mastered} />
              <Stat value={initialData.stats.reviews} label={m.notebook.reviews} />
            </div>
          </div>
          <div className="rounded-3xl bg-[#15292d] p-6 text-white">
            <p className="text-sm font-semibold">{m.notebook.dailyTitle}</p>
            <p className="mt-2 text-sm leading-6 text-white/60">{m.notebook.dailyHint}</p>
...
function AudioPlayButton({ audioUrl, resetKey, messages }: { audioUrl: string; resetKey: number; messages: PlayerMessages }) {
